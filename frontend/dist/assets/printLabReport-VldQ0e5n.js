import{k as q,m as X,b as z}from"./index-ZIFa5WC3.js";import{c as Z}from"./flagHelper-DRXhDmwa.js";import{n as w,M as _}from"./categoryHelper-CWLlTRjy.js";const t=e=>String(e??"—").replace(/[&<>"']/g,d=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[d]),V=e=>e?new Date(e).toLocaleString():"—";function ee(e,d=""){let n=String(e.flag||"").trim().toUpperCase();return!n&&e.result&&e.referenceValue&&(n=Z(e.result,e.referenceValue,d)),["CH","CRITICAL HIGH","CRITICAL_HIGH"].includes(n)?"CH":["CL","CRITICAL LOW","CRITICAL_LOW"].includes(n)?"CL":n==="H"||n==="HIGH"?"H":n==="L"||n==="LOW"?"L":n==="N"||n==="NORMAL"?"Normal":"—"}function te(e,d,n,A,x){var L,E,I,B,M,F,U,G;const y=(e==null?void 0:e.testType)||(e==null?void 0:e.docType)==="PathologyCase"||!!((L=e==null?void 0:e.structuredReport)!=null&&L.grossDescription||(E=e==null?void 0:e.structuredReport)!=null&&E.cytologicalFindings||(I=e==null?void 0:e.structuredReport)!=null&&I.rbcMorphology),c=(e==null?void 0:e.examinationType)||(e==null?void 0:e.docType)==="RadiologyCase"||!!((B=e==null?void 0:e.structuredReport)!=null&&B.liver||(M=e==null?void 0:e.structuredReport)!=null&&M.findings),s=e.patient||{},v=x!==void 0?x:e.showFooter!==void 0?e.showFooter:!0,h=v&&n?`<img src="${n}" alt="ETU Diagnostic Laboratory Logo" style="max-height: 90px; width: auto; max-width: 100%; display: block; margin: 0 auto 10px; object-fit: contain;" />`:v?'<div class="logo">ETU</div>':"",C=s.referralHospital?`<div><b>Referral Hospital Name</b>${t(s.referralHospital)}</div><div><b>Referral Hospital Address</b>${t(A||s.address||"Not recorded")}</div>`:"",W=s.systolicBP||s.diastolicBP?`<div><b>Blood Pressure</b>${t(s.systolicBP||"—")}/${t(s.diastolicBP||"—")} mmHg</div>`:"",O=(s.sampleTypes||[]).map(o=>(o==null?void 0:o.name)||o).filter(Boolean).join(", ")||(y?"Pathology Specimen":c?"Imaging Scan":"Specimen Assigned"),K=V(s.collectionDate||s.registrationDate||s.createdDate||e.createdDate),P=V(e.approvedAt||e.approvedDate||e.approvalDate||e.updatedDate||new Date);let u="",$="Official Laboratory Test Report",S=t(((F=e.technician)==null?void 0:F.fullName)||((U=e.submittedBy)==null?void 0:U.fullName)||(d==null?void 0:d.fullName)||"Clinical Specialist"),H=t(((G=e.approvedBy)==null?void 0:G.fullName)||(e.status==="Approved"?d==null?void 0:d.fullName:"Pending Specialist Approval")),k=e.approverRole||(y?"Pathologist":c?"Radiologist":"Approver / Laboratory Technologist");if(y)if($=`Pathology Examination Report — ${t(e.testType||"Biopsy")}`,k="Pathologist",e.reportType==="Option A"||!e.reportType&&e.reportContent)u=`
      ...
      `;else{const o=e.structuredReport||{};u=`
        <section class="section">
      ...
      `}else if(c)if($=`Radiology & Imaging Report — ${t(e.customExaminationName||(e.ultrasoundSubtype?`Ultrasound — ${e.ultrasoundSubtype}`:e.examinationType||"Diagnostic Imaging"))}`,k="Radiologist",e.reportType==="Option A"||!e.reportType&&e.reportContent)u=`
        <section class="section">
      ...
      `;else{const a=e.structuredReport||{};u=`
        <section class="section">
      ...
      `}else{const o=e.results||[],a={},D={};(Array.isArray(e==null?void 0:e.laboratoryTests)?e.laboratoryTests:Array.isArray(s==null?void 0:s.laboratoryTests)?s.laboratoryTests:[]).forEach(i=>{var T,g;if(!i||typeof i!="object")return;const p=i.category?typeof i.category=="object"?i.category.name||"":String(i.category):"",l=Array.isArray(i.parameters)&&i.parameters.length>0?typeof i.parameters[0]=="string"?i.parameters[0]:((T=i.parameters[0])==null?void 0:T.name)||((g=i.parameters[0])==null?void 0:g.sampleName):i.name,r=w(p,l||i.name),b=i.subcategory||"";Array.isArray(i.parameters)&&i.parameters.forEach(m=>{const N=typeof m=="string"?m:(m==null?void 0:m.name)||(m==null?void 0:m.sampleName)||"";N&&(a[N]=r,b&&(D[N]=b.toUpperCase()))}),i.name&&(a[i.name]=r,b&&(D[i.name]=b.toUpperCase()))});const R=new Map;o.forEach(i=>{const p=w(i.category||a[i.sampleName],i.sampleName),l=(i.subcategory||D[i.sampleName]||"").toUpperCase();R.has(p)||R.set(p,new Map);const r=R.get(p),b=l||"GENERAL";r.has(b)||r.set(b,[]),r.get(b).push(i)});const j=Array.from(R.entries()).sort(([i],[p])=>{const l=_.indexOf(i),r=_.indexOf(p);return l!==-1&&r!==-1?l-r:l!==-1?-1:r!==-1?1:i.localeCompare(p)}),J=Array.isArray(e.testInterpretations)?e.testInterpretations:[],Q=i=>{const p=w(i),l=J.find(r=>w(r.testName)===p);return(l==null?void 0:l.interpretations)||[]};let f="";j.length>0?j.forEach(([i,p])=>{f+=`<div class="result-cat-block" style="margin-bottom: 22px;"><h4 class="result-cat-header">${t(i)}</h4>`,p.forEach((r,b)=>{b!=="GENERAL"&&(f+=`<h5 style="margin: 6px 0 4px 0; font-size: 11px; text-transform: uppercase; color: #075c91; background: #e8f5fa; padding: 3px 8px; border-radius: 4px; display: inline-block;">${t(b)}</h5>`);const T=r.map(g=>{const m=ee(g,s.sex);return`<tr><td><b>${t(g.sampleName)}</b>${g.remarks?`<small>${t(g.remarks)}</small>`:""}</td><td>${t(g.result)}</td><td>${t(g.unit)}</td><td>${t(g.referenceValue)}</td><td><b>${t(m)}</b></td></tr>`}).join("");f+=`<table style="margin-bottom: 6px;"><thead><tr><th>Test / Parameter</th><th>Result</th><th>SI Unit</th><th>Reference Range</th><th>Flag</th></tr></thead><tbody>${T}</tbody></table>`});const l=Q(i);l.length>0&&(f+='<div style="margin: 6px 0 14px 0; padding: 8px 12px; background: #f0f7fa; border-left: 4px solid #075c91; border-radius: 4px;"><b style="color: #075c91; font-size: 11px; text-transform: uppercase;">Clinical Interpretation:</b>',l.forEach(r=>{f+=`<div style="margin-top: 4px; font-size: 11px; color: #203640;"><b>${t(r.title)}:</b> ${t(r.interpretation)}</div>`}),f+="</div>"),f+="</div>"}):f='<table><thead><tr><th>Test / Parameter</th><th>Result</th><th>SI Unit</th><th>Reference Range</th><th>Flag</th></tr></thead><tbody><tr><td colspan="5">No laboratory results recorded.</td></tr></tbody></table>',u=`
      <section class="section">
    ...
    <footer class="footer">
      ...
      <span>Approved by<br><b>Dr. ${H}</b> (${t(k)})</span>
  ...
  <main class="page">
    ...
    <section class="section">
      ...
      <div class="patient">
        ...
        <div><b>Approved By</b>Dr. ${H} (${t(k)})</div>
...
</html>`}async function ne(e,d,n,A){typeof d!="string"&&(n=d||n||q(),d=X()),n||(n=q());const x=typeof e=="string"?e:e==null?void 0:e._id;if(!x)throw new Error("The requested document could not be loaded.");const y=window.open("","_blank","width=980,height=900");if(!y)throw new Error("Print preview was blocked. Please allow pop-ups and try again.");try{let c=typeof e=="object"?e:null,s="",v="";if(!c||!c.patient)try{const h=await z(`/final-reports/${x}`,{token:d});c=h.report,s=h.logoBase64,v=h.referralHospitalAddress}catch{try{c=(await z(`/pathology/cases/${x}`,{token:d})).case}catch{c=(await z(`/radiology/cases/${x}`,{token:d})).case}}y.document.write(te(c,n,s,v,A)),y.document.close()}catch(c){throw y.close(),new Error(c.message||"The requested document could not be loaded.")}}export{ne as p};
