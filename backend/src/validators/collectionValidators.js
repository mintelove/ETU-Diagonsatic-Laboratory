...
export const reportSchema=z.object({equipment:z.array(z.string().trim().min(2).max(120)).min(1).max(5),results:z.array(z.object({sampleName:z.string().trim().min(1).max(120),result:z.string().trim().min(1).max(120),referenceValue:z.string().trim().max(120).optional().default('')})).min(1).max(100),comments:z.string().trim().max(2000).optional().default('')});
