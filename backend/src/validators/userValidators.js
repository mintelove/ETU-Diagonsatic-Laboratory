...
const base = { fullName: z.string().trim().min(2).max(100), username: z.string().trim().toLowerCase().min(3).max(30).regex(/^[a-z0-9._-]+$/, 'Username may contain lowercase letters, numbers, dots, hyphens and underscores only.'), phone, email, role: z.enum(ROLE_VALUES), branchName: z.enum(['Main', 'Otona']).default('Main') };
