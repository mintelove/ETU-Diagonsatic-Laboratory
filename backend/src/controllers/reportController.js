...
export async function getTransactionsReport(req, res, next) {
  try {
    ...
    /* ── Sub Admin: clamp to strict 4-day window ──────── */
    ...
    if (isSubAdmin) {
      // Sub Admin is strictly clamped to maximum 4-day historical window
      if (!startDate || startDate < fourDaysAgo) {
        startDate = fourDaysAgo;
      }
      if (!endDate || endDate > todayEnd) {
        endDate = todayEnd;
      }
      reportMode = 'range';
