...
export const preferenceSchema = z.object({
  ...
  dateFormat: z.enum(['locale', 'iso']).optional(),
  notifications: z.boolean().optional(),
  sidebarCollapsed: z.boolean().optional(),
  customTheme: customTheme.nullable().optional()
...
export async function updatePreferences(req, res, next) {
  try {
    ...
    // If non-admin attempts to set theme to light while allowLightTheme is false, force dark
    if (updates.theme === 'light' && req.user.role !== 'Admin') {
      const allowLightSetting = await SystemSetting.findOne({ key: 'allowLightTheme' }).lean();
      if (!allowLightSetting?.value) {
        updates.theme = 'dark';
      }
    }
